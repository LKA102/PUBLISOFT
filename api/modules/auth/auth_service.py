...
class AuthService:
    ...
    def login_user(self, email: str, password: str):
        ...
        response = global_supabase_client.auth.sign_in_with_password(
    ...
    def register_user(self, email: str, password: str, role: int) -> UserEntity:
        ...
        response = global_supabase_client.auth.sign_up(
            credentials={"email": email, "password": password}
        )
        ...
        user_entity = UserEntity(
            ...
            email=EmailVO(email),
            id_role=role,
            id_state=StatusEnum.ACTIVO.value,
            user_code="".join(
                random.choices(string.ascii_uppercase + string.digits, k=8)
            ),  # This is temporary, should be replaced with a proper user code generation logic
        )
